...
for k=1:length(egitim)
    ...
    while(hata>0.52)
        ...
        for i=1:noronsayisi
            top=0;
            for n=1:girissayisi
               % x(n,k)=min(x(:,k))+round(max(x(:,k))-min(x(:,k)))*0.373626;
                top=top+x(n,k)*w(i,n);
            end
            o(i)=sigmoid(top+b(i));
        end
...
for i=1:noronsayisi
    top=0;
    for n=1:girissayisi
        %t11(n,k)=min(t11(:,k))+round(max(t11(:,k))-min(t11(:,k)))*0.373626;
        
        top=top+t11(n,k)*w(i,n);
    end
    o(i)=sigmoid(top+b(i));
end
